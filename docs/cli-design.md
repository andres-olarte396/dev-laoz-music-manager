...
* **Opciones:**
  ...
  * `--deep`: Lee los *tags* ID3 de cada archivo y recalcula el *hash* criptográfico para detectar corrupciones o ediciones invisibles al sistema de archivos.
...
### `doctor`
Analiza la base de datos contra el disco duro real buscando inconsistencias (rutas rotas, archivos borrados, *hashes* que no coinciden).
...
### `migrate`
Mueve o copia la biblioteca entera a un nuevo destino (ej. un disco duro externo o un NAS) reestructurando por "Artista/Álbum" y calculando *hashes* BLAKE3 en streaming.
...
* **Opciones:**
  ...
  * `--concurrent <N>`: Número de *workers* asíncronos en el pool (Default: Número de núcleos de CPU lógicos).
